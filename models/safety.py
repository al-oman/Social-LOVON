...
- h_decay       : per-step multiplier on H along the trajectory (applied on
                  top of gamma).  Lets you shrink the peak independently of
                  the gamma scaling.
                  h_decay < 1 → peak shrinks along trajectory
                  h_decay = 1 → peak unchanged (default, backward-compatible)
"""
...
# ------------------------------------------------------------------
SIGMA = 2.0          # Gaussian width at current position (meters)
...
H_DECAY = 1.0        # additional per-step H multiplier (1.0 = no extra decay)
...
def _trajectory_grid(X, Y, human_predicted_paths,
                     ...
                     sigma_spread=SIGMA_SPREAD, h_decay=H_DECAY):
...
def _safety_scores(X, Y, human_positions,
    ...
    if human_traj_pred:
        s_traj = _trajectory_grid(X, Y, human_predicted_paths,
                                  sigma, h, gamma, sigma_spread, h_decay)
    else:
